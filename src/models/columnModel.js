...
// Chỉ định ra những field mà chúng ta ko muốn cập nhật trong hàm update()
...
// func push 1 giá trị cardId vào cuối mảng cardOrderIds
...
const update = async (columnId, updateData) => {
  try {
    // Lọc những field ko được cập nhật
    ...
    const result = await GET_DB().collection(COLUMN_COLLECTION_NAME).findOneAndUpdate(
      ...
      { $set: updateData },
      { returnDocument: 'after' } // sẽ trả về kq mới sau khi cập nhật
    )
    return result
  } catch (error) { throw new Error(error) }
}
