...
const createNew = async (reqBody) => {
  try {
    ...
    // Lấy bản ghi board sau khi gọi (tùy mục đích dự án có cần bc này ko)
    const getNewBoard = await boardModel.findOneById(createdBoard.insertedId)

    // Làm thêm các xử lý logic khác với các Collection khác tùy đặc thù dự án...vv
    // Bắn email, notification về cho admin khi có 1 cái board mới đc tạo...vv
...
const getDetails = async (boardId) => {
  try {
    ...
    // B1: Deep Clone board ra một cái mới để xử lý, ko ảnh hưởng tới board ban đầu, tùy mục đích về sau mà có cần clone deep hay ko(vid 63)
